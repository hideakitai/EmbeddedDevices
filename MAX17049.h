...
namespace MAX17049
{
    ...
    public:
        ...
        void hibernate(uint8_t act_th, uint8_t hib_th) { /* TODO: */ write(REG::HIBRT, (((uint16_t)act_th << 8) | (uint16_t)hib_th)); }
    ...
    private:
        ...
        void write(const REG::REG& reg, bool stop = true)
        {
        ...
        void write(const REG::REG& reg, const uint16_t data, bool stop = true)
        {
            Wire.beginTransmission(I2C_ADDR);
            Wire.write(reg);
        ...
        uint16_t read(const REG::REG& reg)
        {
            write(reg, false);
